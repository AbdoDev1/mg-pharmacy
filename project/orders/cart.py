...
class Cart:
    ...
    def _is_allowed_unit(self, unit):
        """
        بوابة الأمان الأولى: الوحدة اللي بتتضاف للسلة لازم تكون فعلاً وحدة
        بيع تابعة لنفس المنتج بتاعها — بغض النظر عن أي unit_id جاي من
        فورم أو طلب HTTP يدوي. من المرحلة 6 (اختيار الوحدة يدويًا)، أي
        وحدة من وحدات المنتج (قطعة أو كرتونة) بقت مسموحة لأي عميل — العميل
        هو اللي بيختارها بنفسه دلوقتي، مش نظام تلقائي بيفرض وحدة واحدة
        حسب نوع حسابه زي قبل كده (Product.units_for_selection هي مصدر
        الحقيقة لقائمة الوحدات المتاحة، نفسها المستخدمة في صفحة تفاصيل
        المنتج لعرض أزرار الاختيار).
        """
        allowed = unit.product.units_for_selection()
