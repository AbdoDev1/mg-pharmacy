...
class Product(models.Model):
    ...
    def units_for_client(self, client):
        """
        الوحدة الافتراضية (وحدة واحدة بس) اللي المفروض تكون **مُختارة مبدئيًا**
        لهذا العميل، حسب الوحدة الافتراضية المحدّدة لنوع حسابه
        (AccountType.default_unit_size):
        ...
        بترجع list من عنصر واحد (أو فاضية لو المنتج مالوش وحدات) عشان تسهّل
        استخدامها في templates بـ {{ list.0 }}.

        ملحوظة (المرحلة 6 — ROADMAP.md): من هنا العميل بقى يقدر يختار أي
        وحدة تانية بنفسه يدويًا في صفحة تفاصيل المنتج — الدالة دي بقت
        بتحدد بس "الاختيار الافتراضي" وقت أول ما الصفحة تفتح، ومش المصدر
        اللي بيحدد الوحدات المسموح بيها في السلة (شوف units_for_selection
        تحت، وorders.cart.Cart._is_allowed_unit). لسه مستخدمة زي ما هي في
        كارت المتجر (product_card.html) عشان السعر الافتراضي المعروض في
        شبكة الكتالوج يفضل زي ما كان (المرحلة 5 اتقفلت من غيرها).
        """
    ...
    def units_for_selection(self):
        """
        **كل** وحدات البيع المتاحة للمنتج ده (مرتبة من الأصغر للأكبر: قطعة
        ثم كرتونة)، بغض النظر عن نوع حساب العميل — المرحلة 6 (ROADMAP.md):
        العميل بقى يختار الوحدة اللي هيشتري بيها بنفسه (بدل اختيار تلقائي
        واحد بس زي units_for_client فوق)، فمحتاجين قائمة كاملة تتعرض كأزرار
        اختيار في صفحة تفاصيل المنتج. مصدر الحقيقة الوحيد لـ"إيه الوحدات
        اللي أصلًا موجودة/مسموحة لهذا المنتج" — orders.cart.Cart._is_allowed_unit
        بيستخدمها هي بالظبط عشان يتأكد إن أي unit_id جاي من فورم/طلب HTTP
        فعلاً تابع لنفس المنتج، قبل ما يتضاف للسلة.
        """
        return sorted(self.units.all(), key=lambda u: u.qty_in_small)
